...
class SawyerCoffeePushEnvV2Display(SawyerXYZEnvDisplay):
    ...
    def evaluate_state(self, obs, action):
        reward, tcp_to_obj, tcp_open, obj_to_target, grasp_reward, in_place = self._compute_reward_coffee_push(
            action, obs)
    ...
    def reset_model(self):
        ...
        if self.random_init and self.num_resets == 0:
            ...
            while np.linalg.norm(pos_mug_init[:2] - pos_mug_goal[:2]) < 0.3:
                pos_mug_goal = self.random_mug_cup_position()
        else:
    ...
    def random_init_coffee_machine_position_and_quat(self, index=None):
        ...
        if all(quat == QUAT_LIST[0]):
            pos_machine = self.goal + np.array([.0, .28, .0])
        elif all(quat == QUAT_LIST[1]):
            pos_machine = self.goal + np.array([-.28, .0, .0])
    ...
    def _compute_reward_coffee_push(self, action, obs):
        obj = obs[4:7]
