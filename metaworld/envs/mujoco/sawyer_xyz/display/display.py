...
class SawyerEnvV2Display(SawyerXYZEnvDisplay):
    ...
    @property
    def model_name(self):
        return full_v2_path_for('sawyer_xyz/_test.xml')
    ...
    def reset_model(self):
        ...
        self.sim.model.body_quat[self.model.body_name2id(
            'drawer'
        )] = np.array([np.sin(np.pi/4), 0, 0, np.sin(np.pi/4)], dtype=np.float64)
        # self.sim.model.body_quat[self.model.body_name2id(
        #     'drawercase_link'
        # )] = np.array([0, 0, 0.7071067811865476, 0.7071067811865476], dtype=np.float64)
        # self.sim.model.body_quat[self.model.body_name2id(
        #     'drawer_link'
        # )] = np.array([0, 0, 0.7071067811865476, 0.7071067811865476], dtype=np.float64)
