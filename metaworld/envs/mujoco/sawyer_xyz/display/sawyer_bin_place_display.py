...
class SawyerBinPlaceEnvV2Display(SawyerXYZEnvDisplay):
    ...
    def __init__(self):
        ...
        self.bin_init_pos = np.array([0.0, 0.6, 0.0])
    ...
    @property
    def model_name(self):
        return full_display_path_for('sawyer_xyz/sawyer_bin.xml')

    @_assert_task_is_set
    def evaluate_state(self, obs, action):
        reward, tcp_to_obj, tcp_open, obj_to_target, grasp_reward, in_place = self.compute_reward_desk_place(
            action, obs)
        ...
        info = {
            ...
            #     self.model.body_name2id('bin')],
            'obj_pos': self.bin_init_pos,
    ...
    def _get_pos_bin(self):
        return self.get_body_com('bin')

    def _get_pos_objects(self):
        return self.get_body_com('obj')

    def _get_quat_bin(self):
        return self.get_body_quat('bin')
    ...
    def _reset_hand(self, steps=50):
        ...
        lower_bound = (-0.0, 0.60, 0.40)
        upper_bound = ( 0.0, 0.60, 0.45)
        self.hand_init_pos = np.random.uniform(lower_bound, upper_bound)
        self.hand_init_pos = upper_bound
        # self.mug_cup_offset = np.array([.00, .5, .3])
        # self.hand_init_pos = self.obj_init_pos + self.mug_cup_offset
    ...
    def reset_objects(self):
        ...
        self.sim.model.body_pos[self.model.body_name2id(
            'bin')] = np.array([-0.0, 0.6, 0.0])
        bin_init_pos = self.sim.model.body_pos[self.model.body_name2id('bin')]
        # self.sim.model.body_pos[self.model.body_name2id(
        #     'bin')] = pos_mug_init + np.array([-0.0, -0.0, -0.2])
        pos_mug_init = bin_init_pos + np.array([0.15, 0.01, 0.0])
        pos_mug_goal = deepcopy(bin_init_pos)

        # self.sim.model.body_pos[
        #     self.model.body_name2id('obj')] = pos_mug_init
        self._set_obj_xyz(pos_mug_init)
        self.obj_init_pos = pos_mug_init
        self.goal = pos_mug_goal
        self._target_pos = pos_mug_goal
        print(self._get_pos_objects())

        print(f'Init: {pos_mug_init}\nGoal: {pos_mug_goal}\n{self._get_pos_objects()}')
        print(f'BinA: {self.sim.model.body_pos[self.model.body_name2id("bin")]}')
    ...
    def _random_init_color(self):
        ...
        for model_name in ['binA', 'mug', 'handle']:
            set_model_rgba(model_name)

    def compute_reward_desk_place(self, action, obs):
        obj = obs[4:7]
