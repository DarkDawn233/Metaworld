...
class SawyerDrawerPickEnvV2Display(SawyerXYZEnvDisplay):
    ...
    def evaluate_state(self, obs, action):
        ...
        target_to_obj,
        object_grasped,
        in_place) = self.compute_reward_drawer_pick(action, obs)
        ...
        info['after_success'] = self._get_after_success_drawer_pick(info)
        after_reward = self._get_after_reward_drawer_pick(info)

        return reward + after_reward, info
    
    def _set_drawer_xyz(self, pos_drawer_link):
    ...
    def _random_xy(self, min_v, max_v):
        x = random.random() * (max_v - min_v) + min_v
        y = random.random() * (max_v - min_v) + min_v
        return np.array([x, y, 0.])

    def _random_init_mug_pos(self):
        if self.drawer_quat_index == 0:
            mug_pos = self.get_body_com('drawer_link') + np.array([.0, -.01, -.09]) + np.array([.0, .0, .0437]) + self._random_xy(-0.005, 0.005)
        elif self.drawer_quat_index == 1:
            mug_pos = self.get_body_com('drawer_link') + np.array([+.01, .0, -.09]) + np.array([.0, .0, .0437]) + self._random_xy(-0.005, 0.005)
        elif self.drawer_quat_index == 2:
            mug_pos = self.get_body_com('drawer_link') + np.array([-.01, .0, -.09]) + np.array([.0, .0, .0437]) + self._random_xy(-0.005, 0.005)
        else:
            mug_pos = self.get_body_com('drawer_link') + np.array([.0, +.01, -.09]) + np.array([.0, .0, .0437]) + self._random_xy(-0.005, 0.005)
    ...
    def reset_model(self):
        ...
        self._set_drawer_xyz(-self.maxDist + (random.random() * 0.01))

        self.mug_init_pos = self._random_init_mug_pos()
        # print("mug_pos:", self.mug_init_pos)
        self._set_mug_xyz(self.mug_init_pos)
    ...
    def _gripper_caging_reward_drawer_pick(self, action, obj_position):
    ...
    def compute_reward_drawer_pick(self, action, obs):
        ...
        object_grasped = self._gripper_caging_reward_drawer_pick(action, obj)
    ...
    def _get_after_success_drawer_pick(self, info):
        hand_pos = self.get_endeff_pos()
        return info['success'] and (hand_pos[2] >= 0.29)

    def _get_after_reward_drawer_pick(self, info):
        if not info['success']:
            return 0
        else:
            hand_pos = self.get_endeff_pos()
