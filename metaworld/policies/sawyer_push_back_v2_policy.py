...
class SawyerPushBackV2Policy(Policy):
    ...
    def _desired_pos(o_d, grab_done, t):
        ...
        if grab_done and t > 20:
            return o_d['goal_pos'] - pos_puck + pos_curr
        elif grab_done:
            return pos_curr
        # If error in the XY plane is greater than 0.02, place end effector above the puck
        if np.linalg.norm(pos_curr[:2] - pos_puck[:2]) > 0.03:
    ...
    def _grab_effort(o_d, grab_done):
        ...
        if grab_done:
            return 0.9, True
        if np.linalg.norm(pos_curr[:2] - pos_puck[:2]) > 0.03 or abs(pos_curr[2] - pos_puck[2]) > 0.055:
            return 0., False
